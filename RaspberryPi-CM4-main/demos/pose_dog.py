...
sport = {"name": "Squat", "count": 0, "calories": 0}
height = 115
quitmark = 0
...
def get_visibility(landmarks):
    if (
        landmarks[mppose.PoseLandmark["RIGHT_HIP"].value].visibility < 0.8
        or landmarks[mppose.PoseLandmark["LEFT_HIP"].value].visibility < 0.8
    ):
        return False
    else:
        return True
    ...
    while not flag:
        ...
        if poseoutput.pose_landmarks:
            mpdraw.draw_landmarks(
                preview, poseoutput.pose_landmarks, mppose.POSE_CONNECTIONS
            )
            ...
            if status:
                cv2.putText(
                    preview,
                    f"{height} : {avg_angle:.1f} {body_ratio:.3f}",
                    (10, 40),
