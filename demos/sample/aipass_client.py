...
def prepare_req_data(request_data):
    ...
    for media_path, media_name in media_path2name.items():
        ...
        media_status = jsonpath.jsonpath(
            request_data, expr="$..payload.{}.status".format(payload_path_list[1])
        )
...
def prepare_ws_data(media_name, payload_path):
    ...
    if media_type in ["audio"]:
        ...
        media_data_list = ne_utils.build_stream_data(
            payload, send_ws=True, read_len=122
        )
    ...
    for i in range(max_length):
        ...
        for media_path, media_content_list in media_list.items():
            ...
            if i <= media_content_loop:
                ...
                request_data["payload"][payload_path_list[1]][payload_path_list[2]] = (
                    base64.b64encode(f_content).decode()
                )
                ...
                if payload_path_list[2] in ["audio", "video"]:
                    tm = jsonpath.jsonpath(
                        request_data,
                        "$.payload.{}.timestamp".format(payload_path_list[1]),
                    )
                    if multi_mode or tm:
                        if payload_path_list[2] == "video":
                            syn_video_timestamp = syn_video_timestamp + time_interval
                            request_data["payload"][payload_path_list[1]][
                                "timestamp"
                            ] = str(syn_video_timestamp)
                        else:
                            syn_audio_timestamp = syn_audio_timestamp + time_interval
                            request_data["payload"][payload_path_list[1]][
                                "timestamp"
                            ] = str(syn_audio_timestamp)
            else:
        ...
        if len(response_match) > 0:
            for response_item in response_match:
                ...
                for media_type in media_type_list:
                    ...
                    write_file_path = "tts.mp3".format(
