...
class XgoExtend(XGO):
    def __init__(self, port, screen, button, cap, baud=115200, version='xgomini'):
        ...
        self.calibration = {}
        try:
            with open("/root/.edublocks/calibration.json", 'r', encoding='utf-8') as f:
                self.k = json.load(f)
    ...
    def prepare_for_block(self, x, y, angle, des_x, emax_x, emax_y, emax_yaw, min_time):
        ...
        if angle > emax_yaw:
            ...
            if abs(y) > emax_y:
                ...
                if abs(e_x) > emax_x:
                    self.move_x_by(e_x, vx=15, mintime=min_time)
                else:
                    print("DONE BLOCK")
                    self.action(0x83)
                    time.sleep(8.5)
    ...
    def search_for_block(self, color, des_x=14, emax_x=1.8, emax_y=1.8, emax_yaw=3.5, mintime=0.72, COUNT_MAX=18):
        count = 0
        length, width, angle, s_x, s_y = 0, 0, 0, 0, 0
        x, y = 0, 0
        self.attitude('p', 10)
        ...
        while True:
            ...
            if isinstance(color, list):
                ...
                contours, img = self.detect_contours(frame, 'cup_' + color)
    ...
    def detect_block_color(self,color=None):
        ...
        while True:
            ...
            if color is None:
                ...
                contours, img = self.detect_contours(frame, 'block_red')
                ...
                contours, img = self.detect_contours(frame, 'block_green')
                flag, _, _, _, _, _ = self.detect_block(contours)
                if flag:
                    green_count += 1
                    lost_count = 0
                ...
                contours, img = self.detect_contours(frame, 'block_blue')
                flag, _, _, _, _, _ = self.detect_block(contours)
                if flag:
                    blue_count += 1
                    lost_count = 0
    ...
    def calibration_block(self, color, COUNT_MAX=25):
        ...
        while True:
            self.check_quit()
            ret, frame = self.cap.read()
            contours, img = self.detect_contours(frame, 'block_' + color)
            if state == 0:
    ...
    #         ret, binary = cv2.threshold(img_gray, 65, 255, cv2.THRESH_BINARY)
    #         img = cv2.cvtColor(binary, cv2.COLOR_GRAY2BGR)
    #         # self.show_img(img)
    #         self.show_img(frame)
    # def search_for_bridge(self, color, COUNT_MAX=18, direction=None, action=0x85):
    #     count = 0
    #     width1, width2, s_y1, s_y2 = 0, 0, 0, 0
    #     x1, x2, y1, y2 = 0, 0, 0, 0
    #     lost_count = 0
    ...
    #         ret, frame = self.cap.read()
    #         contours, img = self.detect_contours(frame, color)
    #         flag, temp_width1, temp_width2, temp_s_y1, temp_s_y2 = self.detect_square(contours)
    #         if not flag:
    #             self.show_img(img)
    #             lost_count += 1
    #             if lost_count > 30:
    ...
    #         s_y1 = (count - 1) * s_y1 / count + temp_s_y1 / count
    #         s_y2 = (count - 1) * s_y2 / count + temp_s_y2 / count
    #         # print("x1: %4.2f, x2: %4.2f, y1: %4.2f, y2: %4.2f"%(width1,width2,s_y1,s_y2))
    #         if count == COUNT_MAX:
    #             count = 0
    #             x1, x2, y1, y2 = self.cal_cup(width1, width2, s_y1, s_y2)
    #             print("x1: %4.2f, x2: %4.2f, y1: %4.2f, y2: %4.2f" % (x1, x2, y1, y2))
    #             done = False
    #             done = self.prepare_for_bridge(x1, x2, y1, y2, action, vx_k=0.029)
    #             if direction is not None:
    #                 self.turn_to(direction, vyaw=30, emax=2)
    #             if done:
    #                 break
    ...
    #             print("x: %4.1f, y: %4.1f, w: %4.1f, h: %4.1f" % (x, y, w, h))
    #         self.show_img(frame)
    ...
    def detect_QR(self, action, count_max=20, direction=None, k=0.035):
        ...
        while True:
            ...
            for i in range(0, len(contours)):
                ...
                # print("x: %4.1f, y: %4.1f, w: %4.1f, h: %4.1f" % (
                #     rect[0][0], rect[0][1], rect[0][1] + rect[1][1], rect[0][0] + rect[1][0]))
        ...
        while True:
            ...
            if count == 0:
                cv2.rectangle(img, (position[0], position[1]), (position[0] + scale, position[1] + scale),
                              (255, 255, 255), 2)
                cv2.putText(img, 'press D(up right)', (40, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
            b, g, r = cv2.split(img)
            img = cv2.merge((r, g, b))
            self.show_img(img)
